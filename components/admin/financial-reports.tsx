...
import { Download, TrendingUp, TrendingDown, DollarSign, CalendarIcon, FileText, Target } from "lucide-react"
import { format } from "date-fns"
import { useToast } from "@/hooks/use-toast"

// Mock financial data
const revenueData = [
  { month: "Jan", revenue: 12500, expenses: 3200, profit: 9300, transactions: 145 },
  { month: "Feb", revenue: 15800, expenses: 4100, profit: 11700, transactions: 189 },
  { month: "Mar", revenue: 18200, expenses: 4800, profit: 13400, transactions: 234 },
  { month: "Apr", revenue: 22100, expenses: 5500, profit: 16600, transactions: 287 },
  { month: "May", revenue: 25600, expenses: 6200, profit: 19400, transactions: 342 },
  { month: "Jun", revenue: 28900, expenses: 7100, profit: 21800, transactions: 398 },
]

const courseRevenueData = [
  { name: "React Fundamentals", revenue: 45600, enrollments: 456, avgPrice: 100 },
  { name: "JavaScript Mastery", revenue: 38900, enrollments: 389, avgPrice: 100 },
  { name: "Python for Beginners", revenue: 32400, enrollments: 432, avgPrice: 75 },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Comprehensive financial analytics and reporting dashboard.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <Progress value={Number.parseFloat(profitMargin)} className="mt-2" />
          </CardContent>
        </Card>
      </div>
                    ...
                    <div key={month.month} className="flex justify-between items-center">
                      ...
                      <div className="flex items-center">
                        ...
                        <span className="text-green-600 font-medium">
                          +{((month.revenue / (revenueData[index] || month).revenue - 1) * 100).toFixed(1)}%
                        </span>
                      </div>
                    </div>
                  ))}
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {course.enrollments} enrollments • Avg: ${course.avgPrice}
                        </p>
                      </div>
                    </div>
                ...
                {regionData.map((region) => (
                  <div key={region.region} className="flex items-center justify-between p-4 border rounded-lg">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{region.percentage}% of total revenue</p>
                    </div>
                    <div className="text-right">
                      <div className="text-xl font-bold">${region.revenue.toLocaleString()}</div>
                      <Progress value={region.percentage} className="w-24 mt-1" />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              ...
              <CardContent className="space-y-4">
                <Button onClick={() => generateReport("Revenue Report")} className="w-full">
                  <Download className="mr-2 h-4 w-4" />
                  Revenue Report (PDF)
                </Button>
                <Button onClick={() => generateReport("Expense Report")} variant="outline" className="w-full">
                  <Download className="mr-2 h-4 w-4" />
                  Expense Report (Excel)
                </Button>
                <Button onClick={() => generateReport("Tax Report")} variant="outline" className="w-full">
                  <Download className="mr-2 h-4 w-4" />
                  Tax Report (CSV)
                </Button>
                <Button onClick={() => generateReport("Profit & Loss")} variant="outline" className="w-full">
