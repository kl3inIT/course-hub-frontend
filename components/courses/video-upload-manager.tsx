...
export function VideoUploadManager({
  ...
  const simulateUpload = async (video: VideoFile): Promise<void> => {
    ...
    setVideos(prev =>
      prev.map(v =>
        v.id === video.id ? { ...v, processingStatus: 'processing' } : v
      )
    )
    ...
    for (let progress = 0; progress <= 100; progress += 10) {
      ...
      setVideos(prev =>
        prev.map(v =>
          v.id === video.id ? { ...v, processingProgress: progress } : v
        )
      )
    }
          ...
          ? {
              ...
              transcoding: {
                formats: [
                  ...
                  {
                    quality: '720p',
                    size: video.file.size * 0.7,
                    url: videoUrl,
                    status: 'completed',
                  },
                  {
                    quality: '480p',
                    size: video.file.size * 0.4,
                    url: videoUrl,
                    status: 'completed',
                  },
                  {
                    quality: '360p',
                    size: video.file.size * 0.2,
                    url: videoUrl,
                    status: 'completed',
                  },
                ],
  ...
  const handleFileSelect = async (files: FileList) => {
    const validFiles = Array.from(files).filter(file => {
      ...
      if (file.size > 500 * 1024 * 1024) {
        // 500MB limit
        alert(
          `File too large: ${formatFileSize(file.size)}. Maximum size is 500MB.`
        )
        return false
      }
      return true
  ...
  const updateVideoMetadata = (
    videoId: string,
    metadata: Partial<VideoFile['metadata']>
  ) => {
    setVideos(prev =>
      prev.map(v =>
        v.id === videoId
          ? { ...v, metadata: { ...v.metadata, ...metadata } }
          : v
      )
    )
  }
                  ...
                  <div className='grid gap-4 md:grid-cols-2'>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={e =>
                          updateVideoMetadata(selectedVideo.id, {
                    ...
                    <div>
                      ...
                      <Input
                        id='video-duration'
                        value={formatDuration(selectedVideo.duration || 0)}
                        readOnly
                      />
                    </div>
                  </div>
                      ...
                      {selectedVideo.metadata.tags.map((tag, index) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() => {
                              const newTags =
                                selectedVideo.metadata.tags.filter(
                                  (_, i) => i !== index
                                )
                              updateVideoMetadata(selectedVideo.id, {
                                tags: newTags,
      ...
      <Dialog open={showMetadataDialog} onOpenChange={setShowMetadataDialog}>
        <DialogContent className='max-w-2xl'>
          <DialogHeader>
            ...
            <DialogDescription>
              Update the title, description, and tags for your video
            </DialogDescription>
          </DialogHeader>
          {selectedVideo && (
            <div className='space-y-4'>
        ...
        <DialogContent>
          ...
          <div className='space-y-4'>
            ...
            <div className='flex justify-end gap-2'>
              ...
              <Button onClick={() => setShowThumbnailDialog(false)}>
                Apply
              </Button>
            </div>
          </div>
