...
import { ArrowLeft, Loader2, ExternalLink, Smartphone, CreditCard, Building, QrCode, Shield } from "lucide-react"
...
export function VNPayPayment({ course, onSuccess, onError, onBack }: VNPayPaymentProps) {
    ...
    if (currentStep === "redirect" && countdown > 0) {
      ...
      setTimeout(() => {
        ...
        if (success) {
          ...
          onError("Payment was cancelled or failed at VNPAY gateway. Please try again.")
        }
      }, 2000)
    }
  ...
  const renderPaymentOptions = () => (
    <div className="space-y-6">
      ...
      <Button onClick={handleProceedPayment} className="w-full" size="lg" disabled={!selectedOption || isProcessing}>
        {isProcessing ? (
          <>
  ...
  const renderProcessing = () => (
    <div className="text-center space-y-6 py-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground">Setting up your secure payment session...</p>
      </div>
    </div>
  )
    ...
    <div className="text-center space-y-6 py-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground mb-4">
          You will be redirected to VNPAY's secure payment gateway in <strong>{countdown}</strong> seconds.
        </p>
        <Alert>
          <AlertDescription>
            <strong>Demo Mode:</strong> This is a simulated VNPAY integration. In production, you would be redirected to
            the actual VNPAY gateway.
          </AlertDescription>
        </Alert>
      </div>
      <div className="space-y-3">
  ...
  const renderConfirming = () => (
    <div className="text-center space-y-6 py-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground">Please wait while we confirm your payment with VNPAY...</p>
      </div>
    </div>
  )
