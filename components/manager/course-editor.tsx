...
const mockCourse: CourseMetadata = {
  ...
  shortDescription: "Learn React.js from basics to advanced with practical projects",
...
const mockVersions: CourseVersion[] = [
  {
    ...
    createdBy: "John Doe",
    changes: ["Updated course description", "Added new learning objectives", "Modified pricing"],
    status: "draft",
  },
  {
    id: "v2",
  ...
  },
  {
    ...
    createdBy: "John Doe",
    changes: ["Initial course creation", "Added basic modules", "Set up course structure"],
    status: "published",
  },
]
  ...
  const validateField = (field: string, value: any): string | null => {
    switch (field) {
      ...
      case "shortDescription":
        if (!value || value.trim().length < 20) return "Short description must be at least 20 characters"
        if (value.length > 160) return "Short description must be less than 160 characters"
        break
      case "price":
      ...
      case "learningObjectives":
        if (!value || value.length === 0) return "At least one learning objective is required"
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">Last updated: {new Date(course.updatedAt).toLocaleDateString()}</p>
        </div>
          ...
          <Dialog open={showPublishDialog} onOpenChange={setShowPublishDialog}>
            ...
            <DialogContent>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button variant={course.isPublished ? "destructive" : "default"} onClick={handlePublish}>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    {validationErrors.shortDescription && (
                      <p className="text-sm text-red-500 mt-1">{validationErrors.shortDescription}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <p className="text-xs text-muted-foreground mt-1">{course.description.length}/2000 characters</p>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <Input
                          ...
                          step="0.01"
                          value={course.price}
                          onChange={(e) => updateCourse("price", Number.parseFloat(e.target.value))}
                          className={`pl-10 ${validationErrors.price ? "border-red-500" : ""}`}
                        />
                      </div>
                      {validationErrors.price && <p className="text-sm text-red-500 mt-1">{validationErrors.price}</p>}
                    </div>
                  ...
                  {course.originalPrice && course.originalPrice > course.price && (
                    <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                      <p className="text-sm text-green-800">
                        ...
                        off (Save ${(course.originalPrice - course.price).toFixed(2)})
                      </p>
                    </div>
                  )}
                </CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Select value={course.visibility} onValueChange={(value) => updateCourse("visibility", value)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Display in featured section</p>
                  </div>
                  <Switch
                    id="featured"
                    checked={course.isFeatured}
                    onCheckedChange={(checked) => updateCourse("isFeatured", checked)}
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Students can leave comments</p>
                  </div>
                  <Switch
                    id="comments"
                    checked={course.allowComments}
                    onCheckedChange={(checked) => updateCourse("allowComments", checked)}
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Students can download resources</p>
                  </div>
                  <Switch
                    id="downloads"
                    checked={course.allowDownloads}
                    onCheckedChange={(checked) => updateCourse("allowDownloads", checked)}
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => updateCourse("certificateEnabled", checked)}
                  />
                </div>
              </CardContent>
            </Card>
                  ...
                  <div className="space-y-2 mt-2">
                    {course.prerequisites.map((prereq, index) => (
                      <div key={index} className="flex items-center gap-2">
                        ...
                        />
                        <Button variant="outline" size="sm" onClick={() => removeArrayItem("prerequisites", index)}>
                          Remove
                        </Button>
                      </div>
                    ))}
                    <Button variant="outline" onClick={() => addArrayItem("prerequisites", "")} className="w-full">
                      Add Prerequisite
                    </Button>
                  </div>
                </div>
                  ...
                  <div className="space-y-2 mt-2">
                    {course.targetAudience.map((audience, index) => (
                      <div key={index} className="flex items-center gap-2">
                        ...
                        />
                        <Button variant="outline" size="sm" onClick={() => removeArrayItem("targetAudience", index)}>
                          Remove
                        </Button>
                      </div>
                    ))}
                    <Button variant="outline" onClick={() => addArrayItem("targetAudience", "")} className="w-full">
                      Add Target Audience
                    </Button>
                  </div>
                </div>
        ...
        <TabsContent value="analytics" className="space-y-6">
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">${(course.enrollmentCount * course.price).toLocaleString()}</div>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-muted-foreground">
                Detailed analytics and performance metrics would be displayed here, including enrollment trends,
                completion rates, student feedback, and revenue analytics.
              </p>
            </CardContent>
          </Card>
        </TabsContent>
      ...
      <Dialog open={showVersionDialog} onOpenChange={setShowVersionDialog}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>View and manage different versions of your course</DialogDescription>
          </DialogHeader>
                ...
                <CardContent className="pt-4">
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      ...
                      <div className="text-xs text-muted-foreground">Created by {version.createdBy}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" onClick={() => revertToVersion(version)}>
