...
import { TrendingUp, Users, BookOpen, DollarSign, Star, Download, RefreshCw, X, Maximize, Minimize } from "lucide-react"
...
const COLORS = ["#8884d8", "#82ca9d", "#ffc658", "#ff7300", "#a4de6c", "#d0ed57", "#8dd1e1", "#d88884"];

// Custom label cho PieChart để tránh dính chữ
const RADIAN = Math.PI / 180;
const renderCustomizedLabel = ({
  cx, cy, midAngle, outerRadius, percent, index, categoryName, percentage
}: any) => {
...
export function ManagerAnalytics() {
  ...
  const filteredCategoryChart = categoryChart.filter(item => item.percentage > 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{totalEnrollments.toLocaleString('en-US')}</div>
            <div className="flex items-center text-xs text-green-600">
              <TrendingUp className="mr-1 h-3 w-3" />
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <div className="space-x-2">
                    <Button size="sm" variant={showDetails ? "outline" : "default"} onClick={() => setShowDetails(false)}>
                      Show Chart
                    </Button>
                    <Button size="sm" variant={showDetails ? "default" : "outline"} onClick={() => setShowDetails(true)}>
                      Show Details
                    </Button>
                  </div>
                </div>
              ...
              <CardContent className="min-h-[500px]">
                {loadingCategory ? (
                  ...
                  <div className={isFullscreenDetail
                    ? "fixed inset-0 z-50 bg-white bg-opacity-95 flex flex-col items-center justify-center p-8 overflow-auto"
                    : "mt-8 relative"
                  }>
                    <div className={isFullscreenDetail ? "w-full max-w-7xl mx-auto" : "w-full max-w-6xl mx-auto"}>
                      <div className="flex justify-between items-center mb-6">
                        ...
                        <div className="flex gap-2">
                          <label className="flex items-center text-sm">
                            ...
                            <span className="ml-2">{isFullscreenDetail ? "Zoom Out" : "Zoom In"}</span>
                          </label>
                          <button
                            className="p-2 rounded-full hover:bg-gray-200"
                            onClick={() => setIsFullscreenDetail(!isFullscreenDetail)}
                            title={isFullscreenDetail ? "Zoom Out" : "Zoom In"}
                          >
                            {isFullscreenDetail ? <Minimize className="h-6 w-6" /> : <Maximize className="h-6 w-6" />}
                          </button>
                        </div>
                      </div>
                      <div className={isFullscreenDetail ? "overflow-y-auto max-h-[70vh] w-full" : "overflow-x-auto w-full"}>
                        <table className={`w-full border bg-white table-fixed ${isFullscreenDetail ? "text-sm" : "text-xs"}`}>
                          <thead>
                            <tr className="bg-gray-50">
                              <th className="px-1 py-1 border text-center" style={{width: '5%', minWidth: '40px'}}>ID</th>
                              <th className="px-1 py-1 border" style={{width: '15%', minWidth: '80px'}}>Name</th>
                              <th className="px-1 py-1 border" style={{width: '15%', minWidth: '90px'}}>Description</th>
                              <th className="px-1 py-1 border text-center" style={{width: '8%', minWidth: '50px'}}>Courses</th>
                              <th className="px-1 py-1 border text-center" style={{width: '8%', minWidth: '60px'}}>Avg Rating</th>
                              <th className="px-1 py-1 border text-center" style={{width: '7%', minWidth: '50px'}}>Students</th>
                              <th className="px-1 py-1 border text-center" style={{width: '10%', minWidth: '70px'}}>Revenue</th>
                              <th className="px-1 py-1 border text-center" style={{width: '12%', minWidth: '80px'}}>Created</th>
                              <th className="px-1 py-1 border text-center" style={{width: '12%', minWidth: '80px'}}>Modified</th>
                            </tr>
                          </thead>
                          <tbody>
                              ...
                              <tr key={cat.categoryId}>
                                <td className="border px-1 py-1 align-top text-center" style={{width: '5%', minWidth: '40px'}}>
                                  {isFullscreenDetail ? (
                                    ...
                                    <div className="line-clamp-2 h-12 overflow-hidden" style={{display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden'}}>
                                ...
                                <td className="border px-1 py-1 align-top" style={{width: '15%', minWidth: '80px'}}>
                                  {isFullscreenDetail ? (
                                    <div>{(cat.categoryName || '').replace(/\n|\r|\r\n/g, ' ')}</div>
                                  ) : (
                                    <div style={{display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', textOverflow: 'ellipsis', lineHeight: '1.2em', maxHeight: '2.4em'}}>
                                ...
                                <td className={`border px-1 py-1 text-center ${isFullscreenDetail ? "text-sm" : "text-xs"}`} style={{width: '12%', minWidth: '80px'}}>{formatDateTime(cat.createdDate)}</td>
                                <td className={`border px-1 py-1 text-center ${isFullscreenDetail ? "text-sm" : "text-xs"}`} style={{width: '12%', minWidth: '80px'}}>{formatDateTime(cat.modifiedDate)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
