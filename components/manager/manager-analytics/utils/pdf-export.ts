...
const addHeader = (doc: jsPDF, exportDateRange?: DateRange, exportTimeRange?: string): void => {
...
const addCategoryTable = (doc: jsPDF, data: any[], yPos: number): number => {
  ...
  autoTable(doc, {
    ...
    body: tableData,
    startY: yPos,
    styles: { fontSize: TABLE_FONT_SIZE, cellPadding: TABLE_CELL_PADDING, font: 'helvetica', halign: 'center', valign: 'middle' },
    headStyles: { ...COMMON_HEAD_STYLE, fillColor: [52, 152, 219], textColor: [255, 255, 255] },
    columnStyles: {
...
const addCourseTable = (doc: jsPDF, data: any[], yPos: number): number => {
  ...
  autoTable(doc, {
    ...
    startY: yPos,
    styles: { fontSize: TABLE_FONT_SIZE, cellPadding: TABLE_CELL_PADDING, font: 'helvetica', halign: 'center', valign: 'middle' },
    headStyles: { ...COMMON_HEAD_STYLE, fillColor: [46, 204, 113], textColor: [255, 255, 255] },
    columnStyles: {
      0: { halign: 'center', cellWidth: widths[0] },
      1: { halign: 'center', cellWidth: widths[1] },
      2: { halign: 'center', cellWidth: widths[2] },
...
const addStudentTable = (doc: jsPDF, data: any[], yPos: number): number => {
  ...
  // @ts-ignore
  autoTable(doc, {
    head: [['No.', 'Course Name', 'New Students', 'Previous', 'Growth %', 'Reviews', 'Avg Rating']],
    body: tableData,
    startY: yPos,
    styles: { fontSize: TABLE_FONT_SIZE, cellPadding: TABLE_CELL_PADDING, font: 'helvetica', halign: 'center', valign: 'middle' },
    headStyles: { ...COMMON_HEAD_STYLE, fillColor: [155, 89, 182], textColor: [255, 255, 255] },
    columnStyles: {
      0: { halign: 'center', cellWidth: widths[0] },
      1: { halign: 'center', cellWidth: widths[1] },
      2: { halign: 'center', cellWidth: widths[2] },
...
const addRevenueTable = (doc: jsPDF, data: any[], yPos: number): number => {
  ...
  autoTable(doc, {
    ...
    startY: yPos,
    styles: { fontSize: TABLE_FONT_SIZE, cellPadding: TABLE_CELL_PADDING, font: 'helvetica', halign: 'center', valign: 'middle' },
    headStyles: { ...COMMON_HEAD_STYLE, fillColor: [241, 196, 15], textColor: [255, 255, 255] },
    columnStyles: {
      0: { halign: 'center', cellWidth: widths[0] },
      1: { halign: 'center', cellWidth: widths[1] },
...
const addFooter = (doc: jsPDF, userName?: string): void => {
  ...
  doc.setFontSize(9);
  doc.text(`Date: ${new Date().toLocaleDateString('en-US')}`, centerX, baseY + 12, { align: 'center' });

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(12);
  doc.text(removeVietnameseTones(userName || ''), centerX, baseY + 28, { align: 'center' });
