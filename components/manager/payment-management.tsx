...
import { DollarSign, Download, Search, Filter, ArrowUpDown, Eye, Calendar } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function PaymentManagement() {
  ...
  const handleExport = () => {
    ...
    if (exportFormat === "csv") {
      // Create CSV content
      const headers = ["ID", "Student", "Course", "Amount", "Status", "Date", "Payment Method"]
      const csvContent = [
        headers.join(","),
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage and track all course payments</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {((paymentData.filter(p => p.status === "completed").length / paymentData.length) * 100).toFixed(1)}% success rate
            </p>
          </CardContent>
        </Card>
            ...
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="cursor-pointer" onClick={() => handleSort('studentName')}>
                  Student {sortConfig?.key === 'studentName' && <ArrowUpDown className="ml-2 h-4 w-4 inline" />}
                </TableHead>
                <TableHead className="cursor-pointer" onClick={() => handleSort('courseName')}>
                  Course {sortConfig?.key === 'courseName' && <ArrowUpDown className="ml-2 h-4 w-4 inline" />}
                </TableHead>
                ...
                <TableHead className="cursor-pointer" onClick={() => handleSort('date')}>
                  Date {sortConfig?.key === 'date' && <ArrowUpDown className="ml-2 h-4 w-4 inline" />}
                </TableHead>
