...
export function PaymentHistory() {
  ...
  useEffect(() => {
    applyFilters()
  }, [transactions, searchTerm, statusFilter, paymentMethodFilter, dateFrom, dateTo])
  ...
  const applyFilters = () => {
    ...
    // Date range filter
    if (dateFrom) {
      filtered = filtered.filter((transaction) => new Date(transaction.date) >= new Date(dateFrom))
    }
    if (dateTo) {
      filtered = filtered.filter((transaction) => new Date(transaction.date) <= new Date(dateTo))
    }
                              ...
                              <div className="space-y-4">
                                ...
                                {selectedTransaction.status === "refunded" && (
                                  <div className="bg-blue-50 p-4 rounded-lg">
                                    <Label className="text-sm font-medium">Refund Information</Label>
                                    <p className="text-sm">
                                      Amount: {formatCurrency(selectedTransaction.refundAmount || 0)}
                                    </p>
                                    <p className="text-sm">Reason: {selectedTransaction.refundReason}</p>
                                  </div>
                                )}
                                <div className="flex justify-end gap-2">
                                  <Button variant="outline" onClick={() => downloadReceipt(selectedTransaction)}>
                                    <Download className="h-4 w-4 mr-2" />
                                    Download Receipt
                                  </Button>
                                </div>
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 pt-4 border-t">
          ...
          <div className="text-sm">
            <span className="font-medium">
              Total:{" "}
              {formatCurrency(
                filteredTransactions.filter((t) => t.status === "completed").reduce((sum, t) => sum + t.amount, 0),
              )}
            </span>
          </div>
        </div>
