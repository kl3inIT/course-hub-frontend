...
const useProfileData = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const errorMessage = error?.response?.data?.message || "Could not fetch user information"
...
export function ProfileSettings() {
  ...
  const { isLoading, error, profileData, setProfileData, updateProfile } = useProfileData()
  const { 
    isChangingPassword, 
    setIsChangingPassword,
    passwordData,
  ...
  if (!user) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-3">
          <p className="text-muted-foreground">Vui lòng đăng nhập để xem thông tin.</p>
          {error && <p className="text-red-500">{error}</p>}
        </div>
      </div>
    )
