  ...
  <Card className="shadow-md">
    <CardHeader className="pb-4">
      <div className="flex flex-col md:flex-row md:items-center md:space-x-6">
        ...
        <div className="mt-4 md:mt-0">
          <CardTitle className="text-2xl md:text-3xl">{userDetail.name}</CardTitle>
          <CardDescription className="text-lg mt-1">{userDetail.email}</CardDescription>
    ...
    <CardContent className="border-t pt-6">
      <div className="grid md:grid-cols-2 gap-8">
        <div>
          <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Bio</h4>
          <p className="mt-2 text-sm text-muted-foreground leading-relaxed">
            {userDetail.bio || "No bio provided"}
          </p>
        </div>
        <div>
          <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Join Date</h4>
...
export function UserDetail({ userId }: { userId: string }) {
  ...
  const enrolledCourses = userDetail.role === "learner" 
          ...
          <Tabs defaultValue="courses" className="w-full">
            <TabsList className="w-full justify-start border-b rounded-none h-14 px-6">
              <TabsTrigger value="courses" className="flex-1 text-base">
                {userDetail.role === "manager" ? "Managed Courses" : "Enrolled Courses"}
              </TabsTrigger>
              ...
              {(userDetail.role === "manager" ? 
                ...
                <div className="text-center py-12">
                  <div className="text-muted-foreground">
                    {userDetail.role === "manager" ? "No managed courses" : "No enrolled courses"}
                  </div>
                </div>
              ) : (
            ...
            {userDetail.role === "learner" && (
              <TabsContent value="activities" className="p-6">
                <div className="space-y-4 max-w-4xl mx-auto">
                  {userDetail.activities.filter(a => a.type === "comment").length === 0 ? (
                    <div className="text-center py-12">
