...
export default function UserProfile() {
  ...
  const completedCourses = MOCK_COURSES.filter(course => course.progress === 100).length
  const inProgressCourses = MOCK_COURSES.filter(course => course.progress > 0 && course.progress < 100).length
              ...
              <Card>
                <CardHeader className="border-b">
                  <div className="flex items-center justify-between">
                    <CardTitle>Enrolled Courses ({MOCK_COURSES.length})</CardTitle>
                  </div>
                </CardHeader>
                <CardContent className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {MOCK_COURSES.map((course) => (
                      <Card key={course.id} className="overflow-hidden hover:border-primary transition-all duration-200">
